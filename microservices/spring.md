# Spring

* https://spring.io/

## Article

* https://ordina-jworks.github.io/spring/2017/06/07/Spring-IO-2017-The-Spring-ecosystem.html
* https://spring.io/blog/2013/11/01/exception-handling-in-spring-mvc
* http://blog.codeleak.pl/2013/11/controlleradvice-improvements-in-spring.html
* https://www.heise.de/developer/meldung/Spring-beginnt-Projekt-fuer-Serverless-Computing-3765938.html
* https://spring.io/guides/gs/accessing-data-jpa/

## Spring Classic

## Spring Reactive

* http://projectreactor.io/
  + https://github.com/reactor/reactor
  + https://github.com/reactor/reactor-core

### Articles

* https://spring.io/blog/2016/11/28/going-reactive-with-spring-data

#### Persistence

* https://docs.spring.io/autorepo/docs/webflow/2.4.3.RELEASE/reference/html/flow-managed-persistence.html
* https://dzone.com/articles/spring-5-webflux-and-jdbc-to-block-or-not-to-block

#### Example apps

* https://github.com/hantsy/spring-reactive-sample (includes kotlin)
* https://github.com/spring-projects/spring-data-examples
* https://spring.io/guides
  + https://github.com/danielyinanc/spring-guides
  + https://github.com/artembilan/gs-reactor-thumbnailer

##### Dedicated sub-projects

* https://cloud.spring.io/spring-cloud-dataflow/
* https://projects.spring.io/spring-data/
  + https://spring.io/guides?filter=spring%20data
* https://github.com/spring-projects/spring-loaded

## Spring

* https://spring.io/blog/

### Quickstart Spring (microservices)

* https://start.spring.io/
* https://github.com/spring-cloud-samples
* https://github.com/hantsy/spring-reactive-sample (also for kotlin)
* https://spring.io/blog/2015/07/14/microservices-with-spring
  + https://github.com/paulc4/microservices-demo

### Subprojects

* https://projects.spring.io/spring-cloud/
  + https://github.com/spring-cloud-samples
    - https://spring.io/projects/spring-cloud-aws
    - https://spring.io/projects/spring-cloud-kubernetes
    - https://spring.io/blog/2020/09/02/hello-azure-spring-cloud
  + https://cloud.spring.io/spring-cloud-sleuth/
* https://docs.spring.io/spring-cloud-dataflow-samples/docs/current/reference/htmlsingle/
* http://www.baeldung.com/the-persistence-layer-with-spring-data-jpa
